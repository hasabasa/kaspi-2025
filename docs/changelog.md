...
## [2025-12-09] - Добавление истории чатов и поддержки файлов для AI-ассистентов
### Добавлено
- Реализована история чатов с сохранением в localStorage (`src/hooks/useChatHistory.ts`)
- Добавлена поддержка загрузки файлов, фото и документов (`src/components/ai-assistants/FileUploadButton.tsx`)
- Обновлен чат-интерфейс для отображения прикрепленных файлов
- Добавлена кнопка "Очистить" для удаления истории чата
- Обновлены типы для поддержки файлов в сообщениях (`ChatFile` интерфейс)
- Обновлены сервисы для отправки файлов через FormData
- Поддержка изображений с предпросмотром в чате
- Возможность скачивания прикрепленных файлов

### Изменено
- `AIChatInterface`: добавлена поддержка файлов и истории
- `AIAccountantPage` и `AILawyerPage`: обновлены для работы с файлами
- Сервисы: добавлена поддержка отправки файлов на бэкенд

### Исправлено
- Нет

## [2025-12-09] - Реализация фронтенда AI-Бухгалтера и AI-Юриста
### Добавлено
- Созданы страницы AI-Бухгалтера и AI-Юриста (`src/pages/AIAccountantPage.tsx`, `src/pages/AILawyerPage.tsx`)
- Создан универсальный компонент чат-интерфейса (`src/components/ai-assistants/AIChatInterface.tsx`)
- Созданы сервисы для работы с API (`src/services/aiAccountantService.ts`, `src/services/aiLawyerService.ts`)
- Созданы TypeScript типы для AI-ассистентов (`src/types/ai-assistants.ts`)
- Добавлены роуты в App.tsx для `/dashboard/ai-accountant` и `/dashboard/ai-lawyer`
- Добавлены пункты меню в Sidebar с иконками Receipt и Scale
- Обновлены API endpoints в `src/config/api.ts`

### Изменено
- Sidebar: добавлены пункты меню "AI-Бухгалтер" и "AI-Юрист"
- App.tsx: добавлены роуты для новых страниц

### Исправлено
- Нет

## [2025-12-09] - Документация AI-бухгалтера и AI-юриста
### Добавлено
- Создана полная документация по реализации AI-бухгалтера и AI-юриста (`docs/ai_accountant_lawyer_implementation.md`)
- Описаны инструменты и технологии для реализации (OpenAI, Supabase Vector, LangChain)
- Определены источники данных (Налоговый кодекс РК, законы, правила Kaspi)
- Предложена архитектура системы с примерами кода
- Создан план внедрения на 4-6 недель
- Описаны API endpoints и структура базы данных
...
## [2025-12-09] - Анализ бэкендов и перенос компонентов
### Добавлено
- Создан сравнительный анализ бэкендов (`docs/backend_comparison.md`)
- Перенесены миграции БД из `kaspi-demper-main/backend` в `unified-backend/migrations/`
- Перенесены скрипты инициализации в `unified-backend/scripts/`
- Перенесена документация по миграциям в `unified-backend/docs/`

### Изменено
- Определен основной бэкенд проекта: `unified-backend` (актуальный)
- Старый бэкенд `kaspi-demper-main/backend` помечен как устаревший

### Исправлено
- Нет

## [2025-12-09] - Загрузка проекта в GitHub и запуск фронтенда
### Добавлено
- Создан файл окружения `.env` для фронтенда на основе `env.frontend.example`
- Проект загружен в GitHub репозиторий: https://github.com/hasabasa/kaspi-2025.git

### Изменено
- Запущен dev-сервер Vite на порту 8080 для локального тестирования
- Обновлен `.gitignore` для исключения больших файлов (Docker.dmg, node установщики)
- Очищена история Git от больших файлов, превышающих лимиты GitHub

### Исправлено
- Удалены большие файлы (Docker.dmg 582MB, node установщики) из репозитория
- Исправлена проблема с превышением лимита размера файлов GitHub (100MB)
