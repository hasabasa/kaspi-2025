...
@app.options("/api/v1/kaspi/stores")
...
async def get_user_stores(user_id: str):
    try:
        ...
        logger.warning(f"Database connection error for user {user_id}: {str(e)}, returning empty stores list")
        return {"success": True, "stores": []}
    except Exception as e:
        error_str = str(e).lower()
        # Если ошибка подключения к БД, возвращаем пустой список вместо 500
        if any(keyword in error_str for keyword in ["connect", "connection", "timeout", "cancelled", "refused"]):
            logger.warning(f"Database connection error for user {user_id}: {str(e)}, returning empty stores list")
            return {"success": True, "stores": []}
...
@app.post("/api/v1/kaspi/stores/{store_id}/sync")
